...
export default class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  _computeEncumbrance(totalWeight, actorData) {
    ...
    totalWeight /= game.settings.get("skjaald", "metricWeightUnits")
      ? CONFIG.SKJAALD.encumbrance.vehicleWeightMultiplier.metric
      : CONFIG.SKJAALD.encumbrance.vehicleWeightMultiplier.imperial;
  ...
  _prepareCrewedItem(item) {
    ...
    item.toggleTitle = game.i18n.localize(`SKJAALD.${isCrewed ? "Crewed" : "Uncrewed"}`);

    // Handle crew actions
    if (item.type === "feat" && item.data.activation.type === "crew") {
      item.cover = game.i18n.localize(`SKJAALD.${item.data.cover ? "CoverTotal" : "None"}`);
  ...
  _prepareItems(data) {
    ...
    const features = {
      ...
      equipment: {
        label: game.i18n.localize("SKJAALD.ItemTypeEquipment"),
        items: [],
        crewable: true,
      ...
      weapons: {
        label: game.i18n.localize("SKJAALD.ItemTypeWeaponPl"),
        items: [],
        crewable: true,
