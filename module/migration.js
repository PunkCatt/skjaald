...
export const migrateWorld = async function() {
  ...
  for ( let a of game.actors ) {
    try {
      ...
      err.message = `Failed skjaald system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items ) {
    try {
      ...
      err.message = `Failed skjaald system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.SKJAALD.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.skjaald ) {
    actorData.flags.skjaald = filterObject(actorData.flags.skjaald, allowedFlags);
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    let res = await fetch("systems/skjaald/json/icon-migration.json");
    data.iconMap = await res.json();
    if ( game.skjaald.isV9 ) {
      res = await fetch("systems/skjaald/json/spell-icon-migration.json");
...
function _migrateActorSenses(actor, updateData) {
  ...
  for ( let s of original.split(",") ) {
    ...
    if ( type in CONFIG.SKJAALD.senses ) {
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("SKJAALD.CreatureSwarm"));
...
const TOKEN_IMAGE_RENAME = {
  "systems/skjaald/tokens/beast/OwlWhite.png": "systems/skjaald/tokens/beast/Owl.webp",
  "systems/skjaald/tokens/beast/ScorpionSand.png": "systems/skjaald/tokens/beast/Scorpion.webp",
  "systems/skjaald/tokens/beast/BaboonBlack.png": "systems/skjaald/tokens/beast/Baboon.webp",
  "systems/skjaald/tokens/humanoid/BanditRedM.png": "systems/skjaald/tokens/humanoid/Bandit.webp",
  "systems/skjaald/tokens/humanoid/GuardBlueM.png": "systems/skjaald/tokens/humanoid/Guard.webp",
  "systems/skjaald/tokens/humanoid/HumanBrownM.png": "systems/skjaald/tokens/humanoid/Commoner.webp",
  "systems/skjaald/tokens/humanoid/NobleSwordM.png": "systems/skjaald/tokens/humanoid/Noble.webp",
  "systems/skjaald/tokens/humanoid/MerfolkBlue.png": "systems/skjaald/tokens/humanoid/Merfolk.webp",
  "systems/skjaald/tokens/humanoid/TribalWarriorM.png": "systems/skjaald/tokens/humanoid/TribalWarrior.webp",
  "systems/skjaald/tokens/devil/Lemure.png": "systems/skjaald/tokens/fiend/Lemure.webp",
  "systems/skjaald/tokens/humanoid/Satyr.png": "systems/skjaald/tokens/fey/Satyr.webp",
  "systems/skjaald/tokens/beast/WinterWolf.png": "systems/skjaald/tokens/monstrosity/WinterWolf.webp"
};
...
const TOKEN_IMAGE_RESCALE = {
  "systems/skjaald/tokens/beast/HunterShark.png": 1.5,
  "systems/skjaald/tokens/beast/GiantElk.png": 1.5,
  "systems/skjaald/tokens/monstrosity/Bulette.png": 1.5,
  "systems/skjaald/tokens/beast/Wolf.png": 1.5,
  "systems/skjaald/tokens/beast/Panther.png": 1.5,
  "systems/skjaald/tokens/beast/Elk.png": 1.5,
  "systems/skjaald/tokens/beast/AxeBeak.png": 1.5,
  "systems/skjaald/tokens/beast/GiantVulture.png": 1.5,
  "systems/skjaald/tokens/beast/GiantSpider.png": 1.5,
  "systems/skjaald/tokens/beast/DireWolf.png": 1.5,
  "systems/skjaald/tokens/monstrosity/DeathDog.png": 1.5,
  "systems/skjaald/tokens/devil/Lemure.png": 1.5,
  "systems/skjaald/tokens/beast/Deer.png": 1.1,
  "systems/skjaald/tokens/beast/GiantWeasel.png": 1.5,
  "systems/skjaald/tokens/beast/Camel.png": 1.2,
  "systems/skjaald/tokens/beast/BloodHawk.png": 1.5
};
...
function _migrateTokenImage(actorData, updateData) {
  ["img", "token.img"].forEach(prop => {
    ...
    if ( img === "systems/skjaald/tokens/humanoid/HumanBrownM.webp" ) {
      updateData[prop] = "systems/skjaald/tokens/humanoid/Commoner.webp";
      return updateData;
    }
    if ( !img?.startsWith("systems/skjaald/tokens/") || img?.endsWith(".webp") ) return;
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.SKJAALD.attunementTypes.NONE;
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.SKJAALD.itemRarity).find(key =>
    (CONFIG.SKJAALD.itemRarity[key].toLowerCase() === item.data.rarity.toLowerCase()) || (key === item.data.rarity)
  );
...
function _migrateItemIcon(item, updateData, {iconMap}={}) {
  if ( iconMap && item.img?.startsWith("systems/skjaald/icons/") ) {
