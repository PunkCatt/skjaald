 ...
 * @param {boolean} [config.skillLearning=false]
 * @param {boolean} [config.spellLearning=false]
...
export async function d20Roll({
  ...
  chooseModifier=false, skillLearning=false, spellLearning=false, fastForward=false, event, template, title, dialogOptions, // Dialog configuration
...
if (!skillLearning && !spellLearning){
...
export async function damageRoll({
  ...
  const roll = new CONFIG.Dice.DamageRoll(formula, data, {
    ...
    criticalMultiplier,
    criticalBonusDamage,
    multiplyNumeric: multiplyNumeric ?? game.settings.get("skjaald", "criticalDamageModifiers"),
    powerfulCritical: powerfulCritical ?? game.settings.get("skjaald", "criticalDamageMaxDice")
  });
