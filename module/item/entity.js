...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.SKJAALD.areaTargetTypes);
  }
   ...
   */
  get isArmor() {
    return this.data.data.armor?.type in CONFIG.SKJAALD.armorTypes;
  }
  ...
  get areEffectsSuppressed() {
    ...
    return this.data.data.attunement === CONFIG.SKJAALD.attunementTypes.REQUIRED;
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      const combat = data.combatFeature;
      const noncombat = data.noncombatFeature;
      if ( combat && !noncombat ) labels.featType = game.i18n.localize("SKJAALD.Combat");
      else if ( noncombat && !combat ) labels.featType = game.i18n.localize("SKJAALD.NonCombat");
      else labels.featType = game.i18n.localize("SKJAALD.CombatNonCombat");
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("SKJAALD.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  prepareFinalAttributes() {
    ...
    if ( this.data.data.hasOwnProperty("actionType") ) {
      ...
      this.labels.abilityCheck = game.i18n.format("SKJAALD.AbilityPromptTitle", {
        ability: CONFIG.SKJAALD.abilities[this.data.data?.ability]
      });
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.SKJAALD.abilities[save.ability];
  ...
  getCriticalThreshold() {
    const itemData = this.data.data;
    const actorFlags = this.actor.data.flags.skjaald || {};
    if ( !this.hasAttack || !itemData ) return;
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    if ( createMeasuredTemplate ) {
      const template = game.skjaald.canvas.AbilityTemplate.fromItem(item);
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/skjaald/templates/chat/item-card.html", templateData);
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["skjaald.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.SKJAALD.attunementTypes.REQUIRED ) {
        props.push(game.i18n.localize(CONFIG.SKJAALD.attunements[CONFIG.SKJAALD.attunementTypes.REQUIRED]));
      }
      props.push(
        game.i18n.localize(data.equipped ? "SKJAALD.Equipped" : "SKJAALD.Unequipped"),
        game.i18n.localize(data.proficient ? "SKJAALD.Proficient" : "SKJAALD.NotProficient")
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.SKJAALD.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("SKJAALD.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.SKJAALD.weaponTypes[data.weaponType]
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.SKJAALD.consumableTypes[data.consumableType],
      `${data.uses.value}/${data.uses.max} ${game.i18n.localize("SKJAALD.Charges")}`
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.SKJAALD.abilities[data.ability] || null,
      CONFIG.SKJAALD.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("SKJAALD.ItemTypeLoot"),
      data.weight ? `${data.weight} ${game.i18n.localize("SKJAALD.AbbreviationLbs")}` : null
    );
  }
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, attackID=null, effectID=null, options={}}={}) {
    ...
    const actorData = this.actor.data.data;
    const messageData = {
      "flags.skjaald.roll": {type: "damage", itemId: this.id},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
  ...
  async rollFormula({spellLevel}={}) {
    ...
    const rollData = this.getRollData();
    if ( spellLevel ) rollData.item.level = spellLevel;
    const title = `${this.name} - ${game.i18n.localize("SKJAALD.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.skjaald.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("SKJAALD.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "SKJAALD.ItemRechargeSuccess" : "SKJAALD.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("skjaald", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("skjaald", "reliableTalent"),
      messageData: {"flags.skjaald.roll": {type: "tool", itemId: this.id }}
    }, options);
  ...
  static async _onChatCardAction(event) {
    ...
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("SKJAALD.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}));
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("SKJAALD.ActionWarningNoToken"));
    return targets;
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.SKJAALD.armorProficienciesMap[this.data.data.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    const weaponProf = CONFIG.SKJAALD.weaponProficienciesMap[this.data.data.weaponType];
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.SKJAALD.sourcePacks.ITEMS}.${CONFIG.SKJAALD.spellScrollIds[level]}`;
