...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen=[]) {
    let data = Object.entries(CONFIG.SKJAALD[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.SKJAALD[`${type}Ids`];
    const map = CONFIG.SKJAALD[`${type}ProficienciesMap`];
    if ( ids !== undefined ) {
    ...
    if ( type === "tool" ) {
      data.vehicle.children = Object.entries(CONFIG.SKJAALD.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
