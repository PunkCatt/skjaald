/**
 * The Skjaald game system for Foundry Virtual Tabletop
 ...
 * Repository: https://gitlab.com/foundrynet/skjaald
 * Issue Tracker: https://gitlab.com/foundrynet/skjaald/issues
 */

// Import Modules
import { SKJAALD } from "./module/config.js";
...
Hooks.once("init", function() {
  console.log(`Skjaald | Initializing the Skjaald Game System\n${SKJAALD.ASCII}`);
  ...
  // This will be removed when skjaald minimum core version is updated to v9.
  if ( !game.skjaald.isV9 ) dice.shimIsDeterministic();
  ...
  Items.registerSheet("skjaald", ItemSheet5e, {
    makeDefault: true,
...
Hooks.once("setup", function() {
  ...
  preLocalizeConfig(CONFIG.SKJAALD, localizeKeys, sortKeys);
  CONFIG.SKJAALD.trackableAttributes = expandAttributeList(CONFIG.SKJAALD.trackableAttributes);
  CONFIG.SKJAALD.consumableResources = expandAttributeList(CONFIG.SKJAALD.consumableResources);
});
...
Hooks.once("ready", function() {
  ...
  const currentVersion = game.settings.get("skjaald", "systemMigrationVersion");
  const NEEDS_MIGRATION_VERSION = "1.5.6";
  const COMPATIBLE_MIGRATION_VERSION = 0.80;
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("skjaald", "systemMigrationVersion", game.system.data.version);
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("skjaald", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("skjaald", "autoCollapseItemCards")) html.find(".card-content").hide();
});
